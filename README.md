...
**Reuse review or diagnosis context.** Accepted, evidence-backed defects use `review-fix` in the eligible existing session that produced the review or diagnosis, including multi-file runtime fixes. A review artifact or analysis/implementation digest or message may supply the findings; a preceding review is not required. Reuse correct and sufficient retained context/output rather than collecting secured evidence again. If faulty, insufficient, or unavailable, start a fresh appropriate workflow (`execute-work-package` for implementation, `author-and-verify-implementation-plan` for technical planning) with a finding-source hint, not claimed inherited context or fresh-session `review-fix`. The Primary's explicit remediation instruction naming accepted defects and scope suffices; no approval token or new Blueprint is required. The Primary may assign defects across multiple eligible source sessions in one bounded pass, each retaining its exact session ID and persona/model variant; source pointers do not transfer retained context. Post-digest Implementer reuse is allowed only through `review-fix` for accepted defects of the same completed package, never another phase/package or indefinite continuation.
...
**Make an invoked review consequential.** `Reduction Required: Yes` or unresolved Critical/Major findings block progression until the primary applies or explicitly rejects them. Accepted conversation-owned plan corrections, including plan reductions, stay primary-owned through `update-plan`; technical implementation-plan/implementation defects follow the `review-fix` eligibility and fallback contract above. Remediation is one bounded pass followed by a compact digest and stop, not an independent review. Escalate materially new risk/uncertainty to the Primary, who decides whether to request independent re-review; never start an automatic review/fix loop.
