...
# an additive layered-profile TOML. The base CODEX_HOME/config.toml is never
# touched; activation is opt-in via the runtime's `-p` or `--profile-v2` flag.
...
# Codex Desktop runtime: codex -p agent-checkpoint
# codex-cli 0.131.0: codex --profile-v2 agent-checkpoint
...
echo "  Before installation, stop every live checkpoint-watch dashboard and every running OpenCode, checkpoint-profile Codex Desktop/CLI, Claude Code, or Hermes session that uses the checkpoint adapter."
...
if [ "$PROJECT_MODE" = false ] && [ "$codex_enabled" = "1" ]; then
    ...
    echo "  Activate with: use the flag exposed by your runtime"
    echo "  Codex Desktop runtime: codex -p agent-checkpoint"
    echo "  codex-cli 0.131.0:     codex --profile-v2 agent-checkpoint"
