...
- **One write boundary**: delegates write only what the loaded skill authorizes; `review-fix` is the single code/test/technical implementation-plan remediation exception. Conversation-owned plan corrections remain with the Primary through `update-plan`.
- **Context-preserving defect remediation**: `review-fix` accepts review artifacts or analysis/implementation digests and messages as finding sources, reusing correct and sufficient retained review/diagnosis evidence instead of collecting it again. Faulty, insufficient, or unavailable context routes to a fresh session with a finding-source hint, not claimed inherited context.
- **Bounded Implementer reuse**: A completed package's Implementer may be resumed after its digest through `review-fix` for accepted defects of that same package only; new phases/packages still require fresh sessions, and the exception cannot become indefinite continuation.
- **Primary-owned remediation decisions**: An explicit instruction naming accepted defects and scope authorizes `review-fix` without a token. The Primary may bundle findings across eligible source sessions and decides on re-review when materially new risk/uncertainty is escalated; each remediation pass stops at its digest, without automatic loops or claims of independent review.
