...
3. **Delegate by task class, not by predicted cost.** Route deterministically:
   ...
   - **`implementer`** — any code change that is not a single-file mechanical edit with an obvious verification step, except eligible existing-session remediation through `review-fix`.
...
| Multi-file or behavioral code edits | `implementer` with Blueprint, unless eligible for `review-fix` | New work needs the execution gate; accepted remediation uses its skill-owned authorization |
...
Resume an existing `delegate-*` `task_id` only for a follow-up on the same thread, a narrower drill-down, small added context, or eligible `review-fix` remediation of defects from its review or diagnosis. Otherwise start a fresh lean task. Implementation-plan authoring always uses one fresh Delegate session per phase; phases stay sequential at the Maintainer and later phase agents read prior artifacts.

For `review-fix`, explicitly instruct remediation of accepted defects within scope and reuse correct and sufficient retained review/diagnosis context to avoid collecting secured evidence again. This includes the narrow post-digest Implementer exception for the same completed package. If context/output is faulty, insufficient, or unavailable, use the skill's fresh-session fallback with a finding-source hint, never claimed inherited context. The Primary may assign a bundled defect set across multiple eligible source sessions; the skill owns exact session/persona/model matching and one-pass closure. No token or automatic review/fix loop is added.
...
- `implementer` — Handles exactly one work package via `execute-work-package` (blueprint → gate → execute → digest), then retires subject only to the same-package `review-fix` exception. Writes **code files only**; no docs/plans or Git.
...
- **Defect remediation** applies accepted conversation-owned plan corrections through `update-plan`; explicitly instructed technical implementation-plan/implementation defects use the `review-fix` routing contract. Review artifacts and analysis/implementation digests or messages are supported sources; step 7 does not require a preceding review. The remediation digest ends the pass. Escalate materially new risk/uncertainty to the Primary, who decides on independent re-review; never create automatic review-fix loops.
