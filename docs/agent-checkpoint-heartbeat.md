...
Das Checkpoint-Tool übernimmt zusätzlich defensible Harness-Telemetrie und meldet Input-Auslastung gegen die allgemeine operative 372k-Grenze, Input-K-Tokens und verbleibende Input-K-Tokens. OpenCode leitet die Werte über `PluginInput.client` aus dem letzten vorherigen abgeschlossenen Assistant-Schritt ab; Codex verwendet beim verifizierten Desktop-Pin das letzte vorherige `token_count`-Transkriptereignis; Claude Code und Hermes verwenden ihre jeweils letzte Harness-Momentaufnahme; Claude Desktop persistiert mangels defensibler Quelle `null`. Feedback kann deshalb hinter dem aktiven Turn zurückliegen. Input- und verbleibende K-Tokens sind reine Laufzeitwerte; der Anteil wird aus Kompatibilitätsgründen im bestehenden Feld `context_used` persistiert.
...
Der Reader-first-Rollout ist für alle fünf Adapter aktiv. Host-Hooks bleiben additiv: OpenCode schreibt bei `session.created`, Codex bei `SessionStart`, Claude Code bei Parent-/Subagent-Start sowie Parent-`SessionEnd`, und Hermes beim neuen Parent-`on_session_start`; der MCP-only Desktop-Adapter besitzt keinen Host-Hook. Unabhängig davon bestätigt jeder erfolgreiche Checkpoint `open`; `close_session=true` deklariert nach genau diesem Checkpoint `closed`. Kein Adapter leitet Status aus Alter, Idle, Tool-Ende oder Prozessende ab.
...
| Codex | Stdio-MCP plus `SessionStart`-/`PreToolUse`-Bridge auf Codex Desktop 26.727.51351; Starts und Checkpoints bestätigen `open`, final deklarierter Close ist möglich, `Stop` bleibt write-free; `agent`/`session_title` immer `null`; standalone `codex-cli` 0.131.0 nicht unterstützt | Native Hook-`session_id` auf Session-Ebene; Children teilen diese Zeile | Letztes vorheriges `token_count`: Input-Anteil/K-Tokens/Headroom aus `last_token_usage.input_tokens` gegen 372k; Cache ist darin bereits enthalten und wird nicht addiert; ungültige, unlesbare oder fehlende Daten bleiben `unknown` |
...
| Claude Desktop | Eigenständiger MCP-only-Adapter mit `checkpoint` und `checkpoint_path`; jeder erfolgreiche Checkpoint schreibt `open` → checkpoint → optional deklariertes `closed` | Explizites vorhandenes absolutes `workspace_root` plus eine einmal pro Konversation erzeugte und stabil wiederverwendete `session_id`; keine Hook-Injektion | `context_used`, `agent` und `session_title` immer ehrlich `null`; Laufzeitwerte `unknown` |
...
MCP kann den gemeinsamen Aufruf `checkpoint(done, next, step_failed, close_session)` transportieren. `close_session` ist optional, standardmäßig `false` und bei Angabe strikt Boolean. Host-integrierte Adapter ergänzen Session-ID und Input-Werte. Claude Desktop kann diese Werte nicht injizieren und erweitert den öffentlichen Aufruf deshalb um `workspace_root` und `session_id`; beide Strings werden vor jedem Schreibzugriff strikt validiert und niemals erfunden.

Die reale Desktop-Grenze wurde mit Claude Desktop 1.24012.9 und dem eingebetteten Commit `03c61d06f8e01a4db2273b9514e225f21d2ba62e` geprüft: Nach vollständigem Quit/Restart war `agent-checkpoint` verbunden und stellte beide Tools bereit; ein tatsächlicher Modellaufruf schrieb exakt `open` → einen achtfeldrigen Checkpoint → `closed`, und Inspector sowie unveränderter Watcher lasen denselben Datensatz als `CLOSED`. Ein direkter MCP-Aufruf, Claude Code, eine gemockte UI oder synthetische JSONL ersetzen diese Modell-E2E-Prüfung nicht.

Der Codex-`PreToolUse`-Hook injiziert Workspace, native Session-ID und den Input-Wert des letzten vorherigen `token_count`-Ereignisses; `SessionStart` schreibt beobachtetes `open`. `last_token_usage.input_tokens` enthält den gecachten Anteil bereits, daher validiert der Hook `cached_input_tokens <= input_tokens`, addiert Cache aber nicht erneut. Ein fehlender, unlesbarer oder malformed Snapshot meldet ehrlich `unknown`. Jeder MCP-Checkpoint ergänzt lazy `open` und optional deklariertes `closed`. Der verifizierte Desktop-Pin besitzt weder einen übernommenen `SessionEnd` noch Child-ID; `Stop`, Exit und Crash bleiben write-free. Subagent-Checkpoints und ihre Closure teilen deshalb das Session-Log, das der nächste Checkpoint wieder öffnet. Ein echter Zwei-Turn-Modelllauf belegte die Kette Modell → Hook → MCP → JSONL → Inspector mit exakten Werten auf beiden Aufrufen; beim zweiten Checkpoint wurden `16379 / 372000 = 0.04402956989247312` persistiert, während die darin enthaltenen `15104` gecachten Tokens nicht erneut addiert wurden. Der separat installierte CLI-Pin 0.131.0 lud im realen Modelllauf zwar MCP, lieferte aber keine konfigurierten Hooks; er ist deshalb nicht als unterstützt dokumentiert.
