...
| Codex | Implementiert: Stdio-MCP-Server `checkpoint`/`checkpoint_path` plus `SessionStart`-/`PreToolUse`-Hook-Bridge (`codex/`, Aktivierung über `codex --profile-v2 agent-checkpoint`); `agent`/`session_title` immer `null` | Native Hook-`session_id` auf Session-Ebene | Immer `null`/`unknown` (kein dokumentierter Telemetrie-Kanal auf dem CLI/MCP-Pfad des gepinnten Builds) |
...
| Claude Code | Implementiert: Skills-Verzeichnis-Plugin `agent-checkpoint@skills-dir` mit gebündeltem Stdio-MCP-Server `checkpoint`/`checkpoint_path` plus `SessionStart`-/`SubagentStart`-/`PreToolUse`-Hook-Bridge (`claude/agent-checkpoint/`, Statusline-Aktivierung über opt-in `claude --settings <home>/agent-checkpoint.settings.json`); `agent` = Subagent-`agent_type` sonst `null`, `session_title` = Statusline-`session_name` sonst `null` | Parent: native Hook-`session_id`; Subagent: komposit `<session_id>--<agent_id>` | Statusline-Sidecar (`.agent-checkpoints/.runtime/claude/`): `used_percentage/100` der letzten Antwort (nur Input), `null` vor erster Antwort/nach Compaction; verbleibende K-Tokens approximativ aus Context-Window minus Input-Tokens; fehlende/ungültige Werte → `null`/`unknown` |
| Hermes | Implementiert: natives User-Plugin mit den Tools `checkpoint`/`checkpoint_path` (`hermes/agent-checkpoint/`, installiert nach `~/.hermes/plugins/agent-checkpoint/`, opt-in über die dokumentierte Aktivierung `hermes plugins enable agent-checkpoint` als additivem `plugins.enabled`-Eintrag); `agent`/`session_title` immer `null` | Native Hook-`session_id` auf Session-Ebene (`on_session_start`/`pre_tool_call`) | Schätzwert aus `pre_api_request`-`approx_input_tokens` geteilt durch bekannte Modell-Context-Grenze, sonst ehrlich `null`/`unknown` |
...
Der implementierte Codex-Adapter nutzt genau diese Trennung: Der `PreToolUse`-Hook injiziert `_workspace_root` (Hook-`cwd`) und `_checkpoint_session_id` (Hook-`session_id`) in den MCP-Tool-Aufruf, der `SessionStart`-Hook liefert die Instruktion. Der gepinnte Build codex-cli 0.131.0 kennt weder einen `SubagentStart`-Hook noch eine `agent_id` auf Hook-Eingaben; Subagent-Checkpoints landen deshalb im selben Session-Log (dokumentiertes Build-Limit). Ein späterer Build mit dokumentierter Subagent-Identität kann Subagent-Logs über eine neue Phase wiederherstellen.

Der implementierte Claude-Code-Adapter nutzt dieselbe Trennung mit dem reicheren Hook-Surface des gepinnten Builds claude 2.1.170: `SessionStart` und `SubagentStart` liefern die Instruktion mit nativem beziehungsweise kompositem `<session_id>--<agent_id>` Checkpoint-ID, und der `PreToolUse`-Hook injiziert `_checkpoint_session_id`, `_telemetry_session_id` (immer die native Session) sowie bei Subagent-Aufrufen `_agent_type` als vollständigen `updatedInput`-Ersatz; vom Aufrufer mitgeschickte interne Felder werden dabei verworfen. Der Workspace-Root stammt aus `CLAUDE_PROJECT_DIR`. Die Statusline schreibt den letzten gültigen Snapshot atomar nach `.agent-checkpoints/.runtime/claude/`; der MCP-Server liest ihn nur bei übereinstimmender Session und Projekt und bildet `null`/Abwesenheit ehrlich als `unknown` ab. Der Sidecar ist ein flüchtiger Latest-Value-Cache und taucht weder in den Roh-Logs noch in der Inspection auf.

Der implementierte Hermes-Adapter ist ein natives Python-User-Plugin (Hermes-Plugins sind Python; keine Node-Abhängigkeit für Hermes-Nutzer) und spiegelt die Kontraktsemantik von `packages/checkpoint-core/src/index.js` (`hermes/agent-checkpoint/agent_checkpoint.py`): Neue Records tragen alle acht Felder mit `agent`/`session_title` gleich `null`, legacy Sechs-Felder-Logs bleiben ohne Migration lesbar, keine abgeleiteten Werte gelangen in die JSONL. Der `on_session_start`-Hook erfasst die native `session_id` und das Arbeitsverzeichnis des Agent-Prozesses (Workspace-Root), `pre_tool_call` bindet dieselbe `session_id` prozessintern an jeden Plugin-Tool-Aufruf (keine model-getragene Identität, kein Argument-Rewriting; ein Mismatch wird sichtbar abgelehnt statt Sessions zu mischen), und `pre_api_request` hinterlegt `approx_input_tokens` plus Modell in einem flüchtigen Latest-Value-Slot, aus dem nur bei bekannter Modell-Context-Grenze ein Schätzwert abgeleitet wird — sonst ehrlich `null`. Der `subagent_start`-Hook existiert auf dem gepinnten Build v0.19.0 (feuert auf `delegate_task`-Kindkonstruktion mit Parent-/Child-Identität), wird aber bewusst nicht übernommen: Session-Level-Logging analog zum Codex-Präzedenzfall, Subagent-Checkpoints teilen das Parent-Log; die Übernahme ist eine dokumentierte Folgeoption. Die Aktivierung folgt dem dokumentierten Opt-in (`hermes plugins enable agent-checkpoint`, ein additiver `plugins.enabled`-Eintrag); der Installer führt genau dieses additive Delta als Textedit aus, damit alle anderen `config.yaml`-Inhalte Byte-für-Byte erhalten bleiben. Hook-/Telemetriezustand ist prozesslokal und wird nie persistiert. `hermes checkpoints` ist Hermes' Shadow-Git-Rollback-Speicher und steht nie als Kontrakt-Ersatz.
