...
The plugin also uses OpenCode's `PluginInput.client` to query session messages and provider models. It selects the latest previous assistant step with positive output tokens and sums TUI-equivalent input, output, reasoning, cache-read, and cache-write fields. Feedback reports that sum as used K-tokens; a matching provider/model limit additionally yields persisted `context_used` and runtime-only remaining K-tokens. The assistant step currently calling the tool is not finalized, so these are previous-completed-step estimates—not active-turn values or compaction headroom. Missing fields degrade independently and never block the checkpoint; used/remaining token values do not enter the exact JSONL schema. See [Agent Checkpoint / Heartbeat](agent-checkpoint-heartbeat.md) for the record contract and selected-log inspection command.
