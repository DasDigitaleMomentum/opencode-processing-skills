...
The plugin uses OpenCode's `PluginInput.client` to query session messages and selects the latest previous assistant step with positive output tokens. That step's `tokens.input` supplies all telemetry: persisted `context_used` is `min(input / 372000, 1)`, runtime input K-tokens are `input / 1000`, and runtime remaining input K-tokens are `max(372000 - input, 0) / 1000`. Provider/model context metadata and non-input token categories are ignored. The assistant step currently calling the tool is not finalized, so all values may lag the active turn. Missing or invalid input produces honest unknowns and never blocks the checkpoint; K-token values do not enter the exact JSONL schema. See [Agent Checkpoint / Heartbeat](agent-checkpoint-heartbeat.md) for the record contract and selected-log inspection command.
