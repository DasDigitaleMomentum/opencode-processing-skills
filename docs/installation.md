...
Restart OpenCode only after starting the newly installed dashboard. The restart loads `checkpoint`, `checkpoint_path`, and the creation-event lifecycle writer. A verified `session.created` event appends `open` for the new native parent or subagent ID. OpenCode exposes no trustworthy resume or graceful-close event on this surface: reopening a preexisting checkpoint-only session remains `UNKNOWN`, and activity, idle, deletion, disposal, age, or errors never synthesize `open` or `closed`. Session logs are written below the active worktree as `.agent-checkpoints/<encoded-session-id>.jsonl`, not below the OpenCode config directory. Ordinary symlinked plugin, runtime, or persona destinations remain preserved; a symlink at the required shared core or watcher reader destinations stops installation loudly before the dependent plugin changes, with recovery guidance.
...
Activate per invocation with `codex --profile-v2 agent-checkpoint`, but only after the compatible dashboard is running. Verify registration with the proof set from [codex/README.md](../codex/README.md): the generated `agent-checkpoint.config.toml` parses, profile layering is accepted (`codex --profile-v2 agent-checkpoint debug prompt-input` exits 0), and the credential-free stdio E2E (SessionStart hook → PreToolUse hook → installed MCP → `checkpoint-inspect` exact eight-field record) passes. Pinned codex-cli 0.131.0 rejects `codex --profile-v2 agent-checkpoint mcp list` — `--profile-v2` only applies to runtime commands and `codex mcp list` has no profile support. The profile enables `[features] hooks = true`, registers the stdio MCP server exposing `checkpoint`/`checkpoint_path`, and adds `SessionStart`/`PreToolUse` command hooks that inject the heartbeat instruction and the native session/workspace identity. Every verified pinned `SessionStart` source (`startup`, `resume`, `clear`, or `compact`) appends an idempotent `open`. The pin has no `SessionEnd`; turn-scoped `Stop` is deliberately ignored, so graceful exit, crashes, and unsupported terminal events leave the latest persisted state `OPEN` rather than fabricating `closed`. New hooks require Codex's hook trust review on first use. The base `~/.codex/config.toml` is left byte-for-byte unchanged. Ordinary unrelated symlink destinations remain preserved, but a symlinked bundled core or whole `~/.codex/agent-checkpoint/` directory is an unverified required dependency: installation stops before hook/profile activation and tells the operator to update the target or deliberately remove/replace the link and rerun. Logging is session-level under the native hook `session_id`; the pinned build exposes no subagent identity, so subagent checkpoints share the session log, and context telemetry is honestly `null`/`unknown`. See [codex/README.md](../codex/README.md) for prerequisites, semantics, and uninstall.
...
The plugin auto-loads as `agent-checkpoint@skills-dir` on the next session (restart Claude Code or run `/reload-plugins` only after the compatible dashboard is running) and passes `claude plugin validate --strict` on exactly the pinned claude 2.1.170 build. It bundles a dependency-free stdio MCP server exposing `checkpoint`/`checkpoint_path` as the scoped tools `mcp__plugin_agent-checkpoint_checkpoint__checkpoint` / `..._checkpoint_path`. `SessionStart` appends parent `open`, `SubagentStart` appends composite-subagent `open`, and both inject the heartbeat instruction with the same checkpoint ID. `SessionEnd` appends `closed` only for the native parent; the pinned surface has no adopted graceful child-end hook, so child logs remain unclosed. `PreToolUse` hooks inject the parent (native `session_id`) or composite subagent (`<session_id>--<agent_id>`) identity into tool calls; new hooks require Claude Code's hook trust approval on first use. A required bundled-core symlink or symlinked path component is preserved but stops installation before any mutation with path-specific recovery guidance.
...
The plugin registers the agent-callable `checkpoint`/`checkpoint_path` tools, injects the complete heartbeat instruction into parent and child turns through `pre_llm_call`, and binds native session/workspace state through `on_session_start`/`pre_tool_call`. Only a verified new-parent `on_session_start` appends parent-owned `open`. A continued session bound through `pre_tool_call` receives no fabricated event and remains `UNKNOWN` when its log is checkpoint-only. `subagent_start` is used only to map each native child internally to its transitive parent-owned log: parent and child invocation/telemetry state stays isolated, while no child identity or attribution is persisted. The pinned v0.19.0 surface has no adopted trustworthy main-session end hook, so Hermes emits no `closed`; child mapping, tool completion, process exit, and age are not substitutes. `context_used` is estimated per native session from `pre_api_request` `approx_input_tokens` only when a defensible model context limit is known, otherwise honestly `null`; remaining headroom never reports below zero; `agent`/`session_title` are always `null`. Note the name collision: `hermes checkpoints` is Hermes' shadow-git rollback store, unrelated to the `.agent-checkpoints/` contract. See [hermes/agent-checkpoint/README.md](../hermes/agent-checkpoint/README.md) for prerequisites, semantics, and uninstall.
...
Refresh and age-reference thresholds are positive integer milliseconds. Defaults are a 1,000 ms refresh and a 120,000 ms informational age reference; CLI values override the `CHECKPOINT_WATCH_REFRESH_MS` and `CHECKPOINT_WATCH_STALE_MS` environment defaults. The columns are `SESSION`, `AGENT`, `NAME/TITLE`, `AGE`, `STATE`, `CHAIN`, `WORK`, `3-WORD`, `CONTEXT`, `DONE`, and `NEXT`. `STATE` is reduced only from explicit `session_status` events: `OPEN` means an observed open with no later close, `CLOSED` means an observed graceful close, and `UNKNOWN` means no status event. Age and the retained stale option never change state, and none of these values proves current process liveness. The three checkpoint-only metrics use `success/count (percent)`, for example `CHAIN 1/1 (100%)`, `WORK 2/3 (66.7%)`, and `3-WORD 6/6 (100%)`; a status-only log shows `0/0 (n/a)` for all three and neutral checkpoint details. `WORK` counts checkpoints with `step_failed=false` rather than proving correct work.
...
Reader-first rollout is active across all four adapters: compatible shared readers and bundled cores are installed before status-capable plugin/hook assets. OpenCode writes `open` only for verified `session.created`; pinned Codex writes `open` for `SessionStart`; neither writes `closed`. Pinned Claude Code writes parent/subagent `open` and native-parent `SessionEnd` `closed`. Pinned Hermes writes only new-parent, parent-owned `open` and never fabricates child attribution or `closed`. Required reader/core symlinks and their symlinked path components stop installation rather than being treated as compatibility evidence. `OPEN` and `CLOSED` are observed-event reductions, not liveness or success claims.
