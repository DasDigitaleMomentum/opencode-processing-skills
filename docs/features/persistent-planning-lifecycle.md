...
The primary maintainer first decides whether persistence is proportional to the work. When it is, the maintainer owns intent and user decisions, while `create-plan` and `author-and-verify-implementation-plan` own the detailed scope and completeness rules for their stages. Their template-governed workflows preserve two-way traceability and stop dependent planning on material user-owned decisions rather than silently inventing them. A bounded self-contained package can instead use an inline `execute-work-package` brief without creating `plans/` artifacts.
...
5. `update-plan` records completed work and evidence-backed phase transitions. It also performs one primary-owned remediation pass for accepted plan-review reductions. Explicitly instructed, evidence-backed implementation-plan defects use one `review-fix` pass in eligible review/diagnosis source sessions; a review artifact is not required. Each returns a digest and stops without automatic re-review.
...
4. `update-plan` identifies the authoritative plan, applies status and changelog updates, validates phase transitions, and performs one bounded primary-owned pass for accepted plan-review findings (`skills/update-plan/SKILL.md:83`). Technical implementation-plan defects instead use one bounded `review-fix` pass when the Primary explicitly supplies accepted defects, evidence, scope, and an eligible source session (`skills/review-fix/SKILL.md:42-53`). Correct and sufficient retained evidence is reused; otherwise a fresh authoring workflow receives only a finding-source hint (`skills/review-fix/SKILL.md:57-62`). Neither path automatically re-reviews.
...
| [Workflow Skills](../modules/workflow-skills.md) | `update-plan` Review Remediation (`skills/update-plan/SKILL.md:83`), `review-fix` Required Inputs and Eligibility (`skills/review-fix/SKILL.md:42-62`), `resume-plan` Workflow (`skills/resume-plan/SKILL.md:45`) | Applies one bounded reduction pass, preserves eligible diagnosis evidence, maintains progress, and reconstructs sessions without automatic re-review loops. |
| [Agent Personas](../modules/agent-personas.md) | `maintainer` Persistent Plan-to-Implementation Lifecycle (`agents/maintainer.md:113`) | Selects persistent versus inline execution, then owns user-facing decisions, phase sequence, and durable tracking. |
| [Agent Personas](../modules/agent-personas.md) | `delegate` How You Work (`agents/delegate.md:42`), `doc-explorer` Core Responsibilities (`agents/doc-explorer.md:32`) | Grounds implementation plans in code and writes planning artifacts only when a governing skill assigns them. |
...
- Plans are conversation-anchored, so material user-owned decisions return through the Maintainer; codebase-answerable and non-observable local technical details remain with the workflow owner.
