...
| `Delegate.Constraints` | policy | public | `agents/delegate.md:61` | Makes the loaded skill's output contract the single write authority, with `review-fix` as the accepted-defect remediation exception, and sets the Git boundary plus lag-aware checkpoint cadence. |
...
| `Implementer frontmatter` | frontmatter | public | `agents/implementer.md:1` | Declares execution-only subagent mode, broad edit permission, leaf-retriever access, and skill access to `execute-work-package`, `browser-walkthrough`, and `review-fix`. |
| `Implementer` | persona | public | `agents/implementer.md:20` | Establishes the execution-only role used by the primary orchestrator. |
| `Implementer.Ground Truth` | section | public | `agents/implementer.md:30` | Makes the execution skill and its templates authoritative for the gated protocol and `review-fix` authoritative for its narrow continuation. |
| `Implementer.Inputs` | section | public | `agents/implementer.md:41` | Accepts persistent plan references or an authoritative inline gated brief, routes uncurated bulk evidence to retriever, and spools verbose EXECUTE output under `/tmp/opencode/`. |
| `Implementer.Modes` | workflow | public | `agents/implementer.md:50` | Separates Blueprint and Execute into distinct primary task calls. |
| `Implementer.MODE: BLUEPRINT` | workflow | public | `agents/implementer.md:54` | Produces the execution step list and optional non-binding Package Sizing Note without commands, edits, or autonomous splitting. |
| `Implementer.MODE: EXECUTE` | workflow | public | `agents/implementer.md:69` | Applies an approved Blueprint in the retained session, requires an approval token, stages targeted tests before the broad/full final gate, and emits the canonical digest. |
| `Implementer.Review-fix Continuation` | workflow | public | `agents/implementer.md:84` | Allows token-free post-digest remediation only for accepted defects of the same completed package under `review-fix` eligibility and one-pass closure. |
| `Implementer.Hard Constraints` | policy | public | `agents/implementer.md:88` | Prohibits Git, preserves the final gate, and checkpoints after approved steps or bounded parts while treating input telemetry as lagging soft guidance. |
| `Implementer.Failure / BLOCKED` | workflow | public | `agents/implementer.md:98` | Defines the minimum execute action and the structured blocked response when execution cannot proceed. |
...
| `Maintainer.Delegate Session Reuse` | policy | public | `agents/maintainer.md:82` | Reuses correct and sufficient review/diagnosis sessions for accepted defects, supports explicit multi-source assignment and same-package Implementer reuse, and routes faulty or unavailable context to a fresh hinted workflow. |
...
| `Maintainer.Delegate Write Boundary` | policy | public | `agents/maintainer.md:94` | Makes the loaded skill's output contract the single write authority, with `review-fix` as the explicitly instructed accepted-defect remediation exception. |
...
5. `delegate` returns analysis or writes an explicitly templated artifact, `doc-explorer` maintains allowed documentation/planning files, `implementer` performs approved code execution, and `legacy-curator` writes only the legacy archive. For `browser-walkthrough`, automated acceptance belongs to Implementer, agent-observed walkthroughs to Delegate, and user-attended walkthroughs are Maintainer-coordinated with optional retained-Delegate execution between interaction points; no new persona or model alias is introduced.
6. Subagents return compact status or digests. Normal BLUEPRINT → EXECUTE reuses one implementer session because approval context is required; after its digest, that session may be resumed only through `review-fix` for accepted defects of the same completed package. Eligible review/diagnosis sessions similarly preserve secured evidence, while faulty, insufficient, or unavailable context routes fresh with a source hint. An interrupted execution with no usable digest is recovered through its selected checkpoint log and current tree into a smaller fresh package. Optional Blueprint sizing remains advisory and Maintainer-controlled. Capacity decisions use lagging input telemetry. Across providers, approximately 205k input tokens are a soft planning signal; at or above 272k, the remaining budget is reserved for a coherent checkpointed exit before the 372k rejection boundary.
...
- OpenCode-installed personas receive the bounded shared instruction from `opencode/checkpoint-instruction.md`. It adds proportional cadence, lag honesty, universal input-based 205k/272k guidance, and the 372k emergency-headroom boundary while preserving final-close lifecycle semantics. Role-specific cadence and interrupted-digest recovery remain in canonical personas. See [OpenCode Checkpoint Adapter](opencode-checkpoint-adapter.md).
