...
1. The installer deploys `maintainer.md` and `maintainer-direct.md` as selectable primary agents and installs the five subagent personas. It may add configured model fields or generate `delegate-*`/`implementer-*` aliases without changing the canonical source body. For OpenCode only, it subsequently appends the marked checkpoint instruction to ordinary installed personas and aliases (`install.sh:945-968`, `install.sh:1088-1090`); Claude/Cursor copies do not advertise the pilot tool.
...
- OpenCode-installed personas receive the shared checkpoint instruction from `opencode/checkpoint-instruction.md:1-9`; unknown telemetry remains unknown and checkpointing is not evidence of work quality. See [OpenCode Checkpoint Adapter](opencode-checkpoint-adapter.md).
