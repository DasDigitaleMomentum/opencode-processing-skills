...
| scriptc, Node.js >=20, clang/toolchain, and Python 3 | optional external | A pre-existing compatible toolchain may compile and fully PTY-test the staged global watcher; absence or any failed diagnostic/smoke retains the Node launch and does not fail installation. |
...
| `Unreleased` | release notes | public | `CHANGELOG.md:5` | Records the browser walkthrough skill, earlier 205k input-planning signal, and skill-owned scope discipline. |
| `0.8.0` | release | public | `CHANGELOG.md:14` | Records cross-harness checkpoints, the Canary, input-budget telemetry, recovery, proportional planning, and multi-harness onboarding. |
| `0.5.0` | release | public | `CHANGELOG.md:60` | Records the unified delegate, review-fix, Cursor, GPT-5.6, scope-discipline, and macOS installer release. |
| `0.2.0` | release | public | `CHANGELOG.md:81` | Records review workflows, configurable variants, project installs, model injection, and execution-policy changes. |
| `0.1.0` | release | public | `CHANGELOG.md:116` | Records the initial skills, agents, installer, README, and license. |
...
| `install_opencode_checkpoint_instruction` | function | internal | `install.sh:1199` | Appends the bounded current block when absent, preserves exact current/symlinked personas, exactly migrates previously current and older known legacy bytes, and refuses unknown marked content unchanged. |
...
6. The effective OpenCode target receives compatible shared core and compact watcher before runtime/plugin. In global mode only, an existing scriptc is then exercised in a disposable source tree: coverage, build, native help/once, non-TTY live refresh/signal/cursor smokes, and Python-standard-library PTY smokes at 80 and 120 columns must all pass before a same-directory temporary executable is atomically renamed to `$HOME/.local/bin/checkpoint-watch`. The PTY gate verifies raw-key refresh, Ctrl-C, cursor cleanup, and exact terminal-flag restoration. Optional failure leaves any previous destination and the portable reader intact; a destination symlink is preserved. Project mode does not inspect this path or probe scriptc.
7. Codex/Claude receive bundled strict-close cores before hooks/runtime, and Hermes receives its Python mirror before enablement. All four installed instructions carry the same proportional cadence, lag honesty, universal input-based 205k/272k guidance, and 372k emergency-headroom intent. OpenCode persona migration recognizes the immediately previous 220k managed block plus older exact known fragments; unrelated configuration and symlink guarantees remain unchanged.
