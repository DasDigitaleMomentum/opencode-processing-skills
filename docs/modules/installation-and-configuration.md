...
7. Codex/Claude receive bundled strict-close cores before hooks/runtime, and Hermes receives its Python mirror before enablement. All four installed instructions carry the same proportional cadence, lag honesty, universal input-based 220k/272k guidance, and 372k emergency-headroom intent. OpenCode persona migration recognizes exact previously current blocks plus older known fragments; unrelated configuration and symlink guarantees remain unchanged.
